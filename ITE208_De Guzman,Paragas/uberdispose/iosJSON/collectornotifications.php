<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        $selectUserSQL = "SELECT * FROM tblrequestrestaurant WHERE collector='".$collector."' AND mark != 'Transaction Finished' ORDER BY date ASC";
        ...
        try {
            if ($sqlLoginResult->num_rows > 0) {
                while ($rows[] = $sqlLoginResult->fetch_assoc()) {
                    $arrayLoginResult = array('CollectorsRequests' => $rows);
